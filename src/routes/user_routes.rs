...
use crate::dtos::{create_user_dto::CreateUserParamsDto, login_user_dto::LoginUserParamsDto};

#[post("/user")]
async fn register_user(data: web::Json<CreateUserParamsDto>) -> impl Responder {
  match CreateUserServiceImpl.execute(
    &CreateUserParamsDto{
...
async fn login_user_controller(data: web::Json<LoginUserParamsDto>) -> impl Responder {
  let user = LoginUserServiceImpl.execute(
    &LoginUserParamsDto{
      email: data.email.to_string(),
      password: data.password.to_string()
    }
